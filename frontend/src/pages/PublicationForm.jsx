...
export default function PublicationForm({ initialProduct, onSubmit }) {
  ...
  return (
    <form onSubmit={submit} className="glass-panel grid gap-4 rounded-lg p-5">
      ...
      <label className="space-y-1">
        <span className="text-sm text-slate-300">Número de WhatsApp</span>
        <div className="flex items-center rounded-md border border-white/10 bg-[#171a1a] focus-within:border-[#ff4b00] overflow-hidden">
          <span className="px-3 py-2 text-sm text-slate-500 border-r border-white/10 select-none">+51</span>
          <input
            className="w-full bg-transparent px-3 py-2 text-sm outline-none"
            value={form.contact}
            onChange={(event) => update('contact', '51' + event.target.value.replace(/\D/g, ''))}
            placeholder="987654321"
            maxLength={9}
          />
        </div>
        <p className="text-xs text-slate-500">Se usará para que el comprador pueda contactarte por WhatsApp.</p>
      </label>
